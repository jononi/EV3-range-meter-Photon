// EV3UARTEmulation.cpp
//
// Emulation of LEGO Mindstorms EV3 UART protocol for creating your own sensors on Particle devices
//
// Author: Lawrie Griffiths (lawrie.griffiths@ntlworld.com) Copyright (C) 2014 Lawrie Griffiths
// Ported for Particle devices by: Jaafar Benabdallah - 2017
...
**/
EV3UARTEmulation::EV3UARTEmulation(USARTSerial *serial, byte type, unsigned long speed) {
  uart = serial;
  this->type = type;
  this->speed = speed;
  status = 0;
    ...
    for(;;) {
      ...
      if (uart->available()) {
        ...
        if (b == BYTE_ACK) {
          ...
          delay(80);
          #ifdef SERIAL_DEBUG
          Serial.println("Received NACK after reset");
          #endif
          last_nack = millis();
          return true;
        }
      }
      // prevent Photon from losing connectivity when not connected to EV3
      Particle.process();
    }
    return false;
  }
  ...
  **/
  void EV3UARTEmulation::heart_beat() {
    if (millis() - last_nack > HEARTBEAT_PERIOD) {
      ...
      /*
      #ifdef SERIAL_DEBUG
      Serial.print(millis() - last_nack);
      Serial.println(" ms elapsed since last NACK\n");
      #endif
      return false; // not getting any NACK from EV3, need to be reset in main
      */
    }
    if (uart->available()) {
      byte b = uart->read();
      ...
      // return true; // there was successful comm with the EV3 brick
    }
  }
