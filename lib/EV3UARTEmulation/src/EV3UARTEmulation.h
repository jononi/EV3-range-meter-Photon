...
/* EV3UARTEmulation.h
 ...
 Description: Emulation of LEGO Mindstorms EV3 UART protocol for creating your
 own sensors on Particle devices

 Credits: Lawrie Griffiths (lawrie.griffiths@ntlworld.com) Copyright (C) 2014:
                           original author of Arduino library
          Jaafar Benabdallah: adapted for Particle Devices (Photon, Electron) and Red Bear Labs Duo

 Changes:
 - heart_beat() and reset() return True or False for the success of the operation
 - reset() no longer blocks Particle's connectivity operations
 - reset() can now be called from loop() and device can be reconnected on EV3 without resetting it
 - added type id in definitions for the compatible UART based sensors to emulate
 - can specify unit symbol when create a new mode

 July - 2017
*/


#ifndef __EV3UARTEMULATION_H__
#define __EV3UARTEMULATION_H__
...
// known compatible EV3 UART sensor types
#define   TYPE_COLOR				29
#define   TYPE_INFRARED			33
#define   TYPE_TOUCH				16
#define   TYPE_ULTRASONIC		30
#define   TYPE_GYRO					32
#define   TYPE_FCL_9DOF			76
#define   TYPE_FCL_ADC			71 //port 1
#define   TYPE_FCL_GESTURE	72
#define   TYPE_FCL_ALTITUDE	74
#define   TYPE_FCL_LIGHT		73
#define   TYPE_FCL_IR				75
#define   TYPE_FCL_HUMIDITY	77
...
class EV3UARTEmulation {
	public:
    EV3UARTEmulation(USARTSerial *serial, byte type, unsigned long speed);
